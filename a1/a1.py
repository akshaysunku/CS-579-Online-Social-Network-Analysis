...
def bfs(graph, root, max_depth):
    ...
    q = deque()
    nodes = graph.nodes()
    seen={v:0 for v in nodes}
    distance= {d:float("inf") for d in nodes}
    parents = {p:[] for p in nodes}
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    node_credit = {n:1 for n in node2distances.keys()}
    edge_credit = defaultdict()
    node_credit[root] = 0
    edges = []
    sort = sorted(node2distances, key=node2distances.get,reverse=True)
    
    # Generate the weights of each node in bottom-up manner
    for ele in sort:
        if(ele != root):
            for parent in node2parents[ele]:
                node_credit[parent] += (node_credit[ele]/len(node2parents[ele]))
                if(ele > parent):
                    edges.append((parent, ele))
                else:
                    edges.append((ele, parent))
    ...
    for edge in edges:
        if( root in edge):
            ...
            if(root_index > 0):
                edge_credit[edge] = (node_credit[edge[0]]/len(node2parents[edge[0]]))
            else:
                edge_credit[edge] = (node_credit[edge[1]]/len(node2parents[edge[1]]))
...
def approximate_betweenness(graph, max_depth):
    ...
    edge_value = defaultdict(int)
    for node in graph.nodes():
        node2distances, node2num_paths, node2parents = bfs(graph, node, max_depth)
        value = bottom_up(node, node2distances, node2num_paths, node2parents)
        
        for edge in value:
            edge_value[edge] = edge_value[edge] + value[edge]
        
    for edge in edge_value:
        edge_value[edge] = edge_value[edge] / 2
...
def partition_girvan_newman(graph, max_depth):
    ...
    approx_betweenness = approximate_betweenness(graph, max_depth)
    sorted_approx_betweenness = sorted(approx_betweenness.items(), key = lambda x: (-x[1],x[0][0],x[0][1]))
    copied_graph = graph.copy()
    
    for edge in sorted_approx_betweenness:
        copied_graph.remove_edge(*(edge[0]))
        if len(get_components(copied_graph)) > 1:
            break
    
    components = sorted(list(get_components(copied_graph)), key=lambda x: -len(x))
    return components
...
def volume(nodes, graph):
    ...
    for node in edge_list:
        if node[0] in nodes or node[1] in nodes:
            count += 1
...
def cut(S, T, graph):
    ...
    edge_list = list(graph.edges())
    for node in edge_list:
        if node[0] in S and node[1] in T:
            count += 1
        elif node[0] in T and node[1] in S:
            count += 1
...
def brute_force_norm_cut(graph, max_size):
    ...
    while size <= max_size:
        for edge in combinations(edge_list, size):
            copied_graph = graph.copy()
            copied_graph.remove_edges_from(edge)
            components = get_components(copied_graph)
            if len(components) == 2:
                result.append((norm_cut(components[0], components[1], graph), list(edge)))
        size += 1
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        result.append((depth, norm_cut(components[0], components[1], graph)))
...
def make_training_graph(graph, test_node, n):
    ...
    neighbors = sorted(graph.neighbors(test_node), key = lambda x: x[0])
    copied_graph = graph.copy()
    i = 0;
    
    while i < n:
        copied_graph.remove_edge(test_node, neighbors[i])
        i = i + 1
...
def jaccard(graph, node, k):
    ...
    for n in nodes:
        if n not in graph.neighbors(node):
            ...
            jaccard = (len(neighbors & neighbors2))/(len(neighbors | neighbors2))
            score_list.append(((node, n), jaccard))
            
    sorted_score_list = sorted(score_list, key = lambda x: (-x[1], x[0][1]))
