    ...
    for i in range(0, len(users)):
        for j in range(i+1, len(users)):
            if(users[i]['screen_name'] == 'HillaryClinton' and users[j]['screen_name'] == 'realDonaldTrump'):
                common_followed = [x for x in users[i]['friends'] if x in users[j]['friends']]
    follow = robust_request(twitter, 'users/lookup', {'user_id': common_followed})
    followed_names = [name['screen_name'] for name in follow]
...
def draw_network(graph, users, filename):
    ...
    nx.draw_networkx_nodes(graph, position, alpha=0.5, node_size=22)
    nx.draw_networkx_edges(graph, position, alpha=0.3, width=0.15)
    nx.draw_networkx_labels(graph, labels=candidates, pos=position,font_size=7)
